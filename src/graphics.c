...
void LadeLBMBild(char* LBMDateiname,unsigned char* Screen,int LoadPal)
{
	...
	BodyPtr=(char *)MemSearch(
		BildDateiPointer,
		BildDateiPointer + stbuf.st_size,
		"BODY");
...
/*@Function============================================================
@Desc: TransparentLadeLBMBild(char*,unsigned char*,int):
	   ...
	   !!! Diese Funktion verwendet den Farbwert TRANSPARENTCOLOR (255) als den
	   Wert, der nicht dargestellt werden soll. !!!

	   !!! Die Transparentfarbe wird derzeit nur bei gepackten Transparent-
	   sequenzen erkannt. Das bedeutet, einzelne Pixel werden vielleicht dennoch
	   dargestellt, obwohl sie 'transparent' sind. !!!

	   !!! Diese Funktion kann das Bild auch im Speicher statt am Bildschrim
	   anzeigen !!!
...
void TransparentLadeLBMBild(char* LBMDateiname,unsigned char* Screen,int LoadPal)
{
	// Variables used by the function
