 ...
 *
 * RotateBulletColor: rotiert die Schu"sfarbe eins weiter
 *
 */
...
void RecFlashFill(int,int,int,char*,int);
int Cent(int);
...
@Int:
* $Function----------------------------------------------------------*/
void RecFlashFill(int LX, int LY, int Color,char* Screen, int SBreite) {
	int i;
	static int num;
...
{
	/*
	Diese Prozedur stellt den momentan intern sichtbaren Kartenausschnitt
	zusammen
...
void ShowPosition(void){
	...
	gotoxy(1,1);
   printf(" CLGrobX : %d ",CLGrobX);
   gotoxy(1,2);
   printf(" CLGrobY : %d ",CLGrobY);
   gotoxy(20,1);
   printf(" CLFeinX : %d ",CLFeinX);
   gotoxy(20,2);
   printf(" CLFeinY : %d ",CLFeinY); 
   gotoxy(1,3);
   printf(" energy : %d ",Me.energy); 
   gotoxy(1,4);
   printf(" X-Absolut: %d   ",Me.pos.x);
   gotoxy(20,4);
   printf(" Y-Absolut: %d   ",Me.pos.y);
#endif
}
...
{
/*
	Diese Prozedur gibt den momentan intern sichtbaren Kartenausschnitt in
	...
	if (Conceptview) {
		...
		for(i=0;i<CurLevel->ylen;i++) {
			for(j=0;j<CurLevel->xlen;j++) {
				SmallBlock(LX,LY,GetMapBrick(j*BLOCKBREITE,i*BLOCKHOEHE),
...
{
/*
	Diese Funktion malt den Influencer an die Position die das Zentrum
...
#ifdef SAYINFLUENCEPOS
	gotoxy(1,1);
	printf("X: %d  Y: %d  ",Me.pos.x,Me.pos.y);
#endif

	/* Offset des Exakten Bildmittelpunktes abzueglich der Eckenverschiebung */
	InternFensterOffset=(INTERNHOEHE)*BLOCKHOEHE/2*BLOCKBREITE*INTERNBREITE+
		INTERNBREITE*BLOCKBREITE/2- BLOCKBREITE/2 -
		BLOCKBREITE*INTERNBREITE*BLOCKHOEHE/2;
		
	/* Verschiebung des Fensters in X- und Y-Richtung */
 	InternFensterOffset+=(((Me.pos.y) % BLOCKHOEHE) - BLOCKHOEHE/2)*
			BLOCKBREITE*INTERNBREITE +
			(Me.pos.x % BLOCKBREITE) - BLOCKBREITE/2;
		
	/* Find the center of screen */
	target = InternWindow + InternFensterOffset;
	
	source = Influencepointer+(Me.phase)*BLOCKMEM;

	// Darstellen des Schildes
	if (!BeamLine) {
		for(i=0;i<4;i++) if (Me.Shield[i]) MergeBlockToWindow(ShieldBlocks+i*BLOCKHOEHE*BLOCKBREITE, target, INTERNBREITE*BLOCKBREITE, FALSE);
//		if (Me.Shield) MergeBlockToWindow(ShieldBlocks+SHIELD1*BLOCKBREITE*BLOCKHOEHE, target, INTERNBREITE*BLOCKBREITE, FALSE);
//		if (Me.LUShield) MergeBlockToWindow(ShieldBlocks+SHIELD2*BLOCKBREITE*BLOCKHOEHE, target, INTERNBREITE*BLOCKBREITE, FALSE);
//		if (Me.LOShield) MergeBlockToWindow(ShieldBlocks+SHIELD3*BLOCKBREITE*BLOCKHOEHE, target, INTERNBREITE*BLOCKBREITE, FALSE);
	}
	
/* Druidmap[Me.type].picpointer+(Me.phase)*BLOCKMEM; */

	if (BeamLine > BLOCKBREITE/2) { BeamLine--; return; }
	if (!BeamLine) MergeBlockToWindow(source, target, INTERNBREITE*BLOCKBREITE, FALSE);
	else {
		for (i=0;i<BLOCKHOEHE;i++) {
...
void PutEnemy(int Enum){
	...
	if (Feindesliste[Enum].energy>100) {
		PutObject(enemyX, enemyY, ShieldBlocks+BLOCKBREITE*BLOCKHOEHE, FALSE);
	}
...
{
/*
	Diese Prozedur schreibt das im Speicher zusammengebaute Bild in den Bild-
   schirmspeicher.
...
void PutEnergieLevel(void){
unsigned char* Energiepointer;
int i;
int j;
	Energiepointer=MK_FP(0x0A000,0);
	Energiepointer+=ENERGIEPOSY*SCREENBREITE;
	for (i=0;i<ENERGIEBALKENBREITE;i++) {
		for (j=0;j<MAXENERGIEBALKENLAENGE;j++) {
			if (j<Me.energy) *Energiepointer=j; else *Energiepointer=15;
			Energiepointer++;
		}
		Energiepointer+=SCREENBREITE-MAXENERGIEBALKENLAENGE;
	}
}
