...
ShowCurrentSkill( void )
{
    ...
    if ( use_open_gl )
    {
	blit_open_gl_texture_to_screen_position ( &SpellSkillMap [ Me[0].readied_skill ] . icon_surface , Target_Rect . x , Target_Rect . y , TRUE ) ;
    }
    else
	our_SDL_blit_surface_wrapper ( SpellSkillMap [ Me[0].readied_skill ] . icon_surface . surface , NULL , Screen , &Target_Rect );
...
#define AVERAGE_LINES_IN_MESSAGE_WINDOW 3*GameConfig . screen_height/480
    ...
    Subtitle_Window . x = ( ( 100 + 25 ) * GameConfig . screen_width ) / 640 ;
    Subtitle_Window . y = ( ( 380 + 10 ) * GameConfig . screen_height ) / 480 ;
    Subtitle_Window . w = ( ( 500 - 20 - 25 ) * GameConfig . screen_width ) / 640 ;
    Subtitle_Window . h = ( 58 * GameConfig . screen_height ) / 480 ;
